...
def translate(file_contents, i_column, separator):
    ...
    for i, line in enumerate(tqdm(file_contents)):
        ...
        utterance = line.split(separator)[column_indx] 

        translated_utterance = translator.translate(utterance, src='de', dest='en')
...
def main(args):
    ...
    if os.path.isfile(source_location):
        ...
        updated_file_contents = translate(file_contents, i_column, separator)
        updated_file_name = file_name.split('.table')[0] + '_transl.table'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Extract text column from table/text \
        files.')
